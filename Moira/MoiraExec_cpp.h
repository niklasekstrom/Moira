...
Moira::execShiftEa(u16 op)
{
    ...
    u32 ea, data;
    future dataFu;
    if (!readOp<M,S, STD_AE_FRAME>(src, ea, dataFu)) return;

    prefetch<POLLIPL>();

    data = getFutureValue(dataFu);
...
Moira::execAbcd(u16 opcode)
{
    ...
    switch (M) {
        ...
        default: // Ea
        {
            ...
            future data1Fu;
            if (!readOp<M,S>(src, ea1, data1Fu)) return;
            pollIpl();
            future data2Fu;
            if (!readOp<M,S,IMPLICIT_DECR>(dst, ea2, data2Fu)) return;

            prefetch();

            data1 = getFutureValue(data1Fu);
            data2 = getFutureValue(data2Fu);
...
Moira::execAddRgEa(u16 opcode)
{
    ...
    future dataFu;
    if (!readOp<M,S, STD_AE_FRAME>(dst, ea, dataFu)) return;
...
Moira::execAddiRg(u16 opcode)
{
    ...
    u32 ea, data, result;
    future dataFu;
    if (!readOp<M,S>(dst, ea, dataFu)) return;
    prefetch<POLLIPL>();
...
Moira::execAddxEa(u16 opcode)
{
    ...
    future data1Fu;
    if (!readOp<M,S, flags>(src, ea1, data1Fu)) {
    ...
    future data2Fu;
    if (!readOp<M,S, flags | IMPLICIT_DECR>(dst, ea2, data2Fu)) {
...
Moira::execBitDxEa(u16 opcode)
{
    ...
    switch (M) {
        ...
        default:
        {
            ...
            u32 ea, data;
            future dataFu;
            if (!readOp<M, Byte>(dst, ea, dataFu)) return;
...
Moira::execCmpm(u16 opcode)
{
    ...
    u32 ea1, ea2, data1, data2;
    future data1Fu;
    if (!readOp<M,S, AE_INC_PC>(src, ea1, data1Fu)) return;
    pollIpl();
    future data2Fu;
    if (!readOp<M,S, AE_INC_PC>(dst, ea2, data2Fu)) return;
...
Moira::execMove8(u16 opcode)
{
    ...
    if (isMemMode(M)) {
        ...
        future dataFu;
        if (!readOp <M,S> (src, ea, dataFu)) return;
    ...
    switch (M) {
        ...
        case 3: // (An)+
        {
            ...
            for(int i = 0; i <= 15; i++) {
                ...
                if (mask & (1 << i)) {
                    writeR(i, SEXT<S>(getFutureValue(fus[i])));
                }
            }
            writeA(src, ea);
            break;
...
Moira::execMul(u16 opcode)
{
    ...
    future dataFu;
    if (!readOp <M,Word, STD_AE_FRAME> (src, ea, dataFu)) return;
...
Moira::execMulMusashi(u16 op)
{
    ...
    future dataFu;
    if (!readOp <M,Word> (src, ea, dataFu)) return;
...
Moira::execDiv(u16 opcode)
{
    ...
    future divisorFu;
    if (!readOp <M,Word, STD_AE_FRAME> (src, ea, divisorFu)) return;
    divisor = getFutureValue(divisorFu);
...
Moira::execDivMusashi(u16 opcode)
{
    ...
    u32 ea, divisor, result;
    future divisorFu;
    if (!readOp<M, Word>(src, ea, divisorFu)) return;
    divisor = getFutureValue(divisorFu);
...
Moira::execNbcd(u16 opcode)
{
    ...
    switch (M) {
        ...
        default: // Ea
        {
            u32 ea, data;
            future dataFu;
            if (!readOp<M, Byte>(reg, ea, dataFu)) return;
            prefetch<POLLIPL>();
            data = getFutureValue(dataFu);
...
Moira::execRtr(u16 opcode)
{
    ...
    bool error;
    u16 newccr = (u16)getFutureValue(readM<M, Word>(reg.sp, error));
    if (error) return;
...
Moira::execRts(u16 opcode)
{
    ...
    bool error;
    u32 newpc = getFutureValue(readM<M, Long>(reg.sp, error));
    if (error) return;
...
Moira::execTst(u16 opcode)
{
    ...
    u32 ea, data;
    future dataFu;
    if (!readOp<M,S, STD_AE_FRAME>(rg, ea, dataFu)) return;
...
Moira::execUnlk(u16 opcode)
{
    ...
    u32 ea, data;
    future dataFu;
    if (!readOp<MODE_AI, Long, AE_DATA|AE_INC_PC|POLLIPL>(7, ea, dataFu)) return;
